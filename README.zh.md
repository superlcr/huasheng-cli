...
这台机器没有浏览器(SSH、云主机、容器)?跑 `hs auth login --no-browser`,在任意设备上打开它打印的地址并授权,
再把浏览器最后跳到的地址粘回来(以 `http://127.0.0.1` 开头、页面打不开,这是正常的)。
...
或者用自己录的口播开始 —— 本机文件或公网地址都行:

```bash
hs make --audio ./口播.m4a --out ./video.mp4
hs make --audio ./口播.m4a --transcript @逐字稿.txt --out ./video.mp4   # 有逐字稿就加上
```

花生会保留你的声音,按它去配画面。逐字稿可选,不给就由花生自己转写。支持
mp3、wav、flac、mp4、m4a;上传由 `hs` 自己完成,不需要、也不会要求你提供任何内部存储地址。
...
> 第 2 个分镜用我自己的文件 ./b-roll.mp4

> 换一个更温暖的旁白音色,先告诉我要多少花生米

确认分镜方案会扣花生米,投稿会发布到公网。`hs` 把这两个和其它单向工具(删项目、删素材、删偏好、加入快速通道)
都标为不可撤销,会对这类工具做确认的客户端会先问你;`hs` 自己不弹确认。生产后换音色、往素材库加素材也扣花生米,
MCP 工具问一下就会先报价。
...
- 确认分镜方案会扣花生米,投稿会发到公网。生产后换音色、往素材库加素材同样扣花生米,扣了不退。`hs` 做命令说的事、做完报花了多少;要不要先问一声,由你、你的脚本或你的 AI 客户端决定。
- 文稿、口播音频和素材会上传到花生用于生成视频;`hs` 没有独立遥测或后台自动更新。唯一的额外连接是每天最多一次
  向 GitHub 查最新版本号,只带 hs 自己的版本、可以关掉。
...
安装器装的会下载最新版、校验 SHA256 后替换;npm 装的会执行 `npm i -g @superlcr/hs@latest`。`hs` 从不自己下载或替换自己。
每天最多一次它会问一下最新版本号,你的版本旧了就提示一句;`HS_NO_UPDATE_CHECK=1` 可以关掉。

用 `hs mcp serve` 的 AI 客户端会一直跑着旧进程,直到它自己重启。升级后在 Claude Code 或 Codex 里开一个新会话,
Claude Desktop 则退出重开;在那之前客户端用的还是旧版本。客户端运行中升级在各平台都是安全的。
